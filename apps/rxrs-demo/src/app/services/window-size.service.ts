...
export class WindowSizeService {
  ...
  constructor(private zone: NgZone) {
    ...
    this.currentSizeSubject = new BehaviorSubject(
      <FullScreenWidthEnum>FullScreenWidthEnum.medium
    );
    ...
    this.rxrs = new RxRs();
    this.rxrs
      .observe(`(max-width: ${screenSizes.small}px)`)
      .subscribe((matches) => {
        if (matches) {
    ...
    this.rxrs
      .observe(
        `(min-width: ${screenSizes.small}px) and (max-width: ${
          screenSizes.medium - 1
        }px)`
      )
      .subscribe((matches) => {
        if (matches) {
    ...
    this.rxrs
      .observe(
        `(min-width: ${screenSizes.medium}px) and (max-width: ${
          screenSizes.large - 1
        }px)`
      )
      .subscribe((matches) => {
    ...
    this.rxrs
      .observe(
        `(min-width: ${screenSizes.large}px) and (max-width: ${
          screenSizes.xLarge - 1
        }px)`
      )
      .subscribe((matches) => {
        if (matches) {
    ...
    this.rxrs
      .observe(`(min-width: ${screenSizes.xLarge}px)`)
      .subscribe((matches) => {
        if (matches) {
          this.screenSizeSubject.next({
            ...initialScreenSizes,
